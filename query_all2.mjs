...
snap.forEach(d => {
    const data = d.data();
    if (
        data.name?.toLowerCase().includes('50') ||
        data.name?.toLowerCase().includes('250') ||
        data.name?.toLowerCase().includes('ind') ||
        data.name?.toLowerCase().includes('inndra') ||
        data.category === 'CBD & Bienestar' ||
        data.category === 'Salud'
    ) {
        all.push({ id: d.id, name: data.name, category: data.category, url: data.imageUrl });
    }
});
