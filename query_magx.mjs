...
snap.forEach(d => {
    const data = d.data();
    if (data.name?.toLowerCase().includes('mag') || data.slug?.toLowerCase().includes('mag')) {
        magx.push({ id: d.id, name: data.name, imageUrl: data.imageUrl, category: data.category, slug: data.slug });
    }
});

console.log(JSON.stringify(magx, null, 2));
process.exit(0);
